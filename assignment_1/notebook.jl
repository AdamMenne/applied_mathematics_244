...
# ╔═╡ 215cbe9e-182b-11ed-1f3f-513b1fe4d02d
using PlutoUI, CairoMakie, DifferentialEquations, ParameterizedFunctions

# ╔═╡ 98a1f674-8f9f-4f73-9e2e-b6c849bafb41
md"# Applied Mathematics 244 - Assignment 1"

# ╔═╡ cba37a2e-c056-40ea-b6a1-d5c47fd14776
md"## Problem 2"

# ╔═╡ e5fabdce-4612-44d4-8750-029771c8545b
begin
	f(t, y) = ℯ^(-2t) * y
	a = 0
	b = 2
...
# ╔═╡ 11a2cd89-a7e3-47c3-982e-9b4cf88a9994
y_e(t) = 2ℯ^(1//2*(1-ℯ^(-2*t)));

# ╔═╡ 6ae6e503-e926-4064-9fd0-4399770c0e45
for i in 1:n
	y[1] = 2
	t[i+1] = t[i] + h
	y_s[i+1] = y[i] + h * f(t[i], y[i])
	y[i+1] = y[i] + h/2 * (f(t[i], y[i]) + f(t[i+1], y_s[i+1]))
end	

# ╔═╡ 4f4e077c-6397-422b-9d1f-d64b0feec33b
for i in 1:n
	y2[1] = 2
	t2[i+1] = t2[i] + h
...
# ╔═╡ d37040e8-4653-4d14-ad45-ab6f32bfd790
y2 # eulers

# ╔═╡ 69cd59d0-78fd-48f4-935b-cf6d8898c1e6
y # modified

# ╔═╡ 2041c8a6-b1ac-4bf6-8fe7-bf0b0ed4d1aa
begin
    fig1 = Figure()
    ax1 = Axis(fig1[1, 1],
		xlabel = "x",
	...
	s1 = scatter!(t,y2)
	s2 = scatter!(t,y)
	l = lines!(range(0, b, length=100),y_e)
	axislegend(ax1,
    [s1, s2, l],
    ["Euler's method", "Modified Euler's method", "Exact solution"], position = :rb)
	fig1
end

# ╔═╡ 7f5cfabd-d4c1-4cb8-8fd8-a8ab75bb6cdd
#save("euler.pdf", fig1)
...
manifest_format = "2.0"
project_hash = "95921a55ab7698d61217537f1ad2e927f98464d0"
...
git-tree-sha1 = "b02c21dd08254dcee5611f50010243ab9060d20f"
...
git-tree-sha1 = "010c9e4b4e120050aa677125b65705cf896c0d51"
...
git-tree-sha1 = "809443c22610ec82e66b5ec60692b75956df0792"
