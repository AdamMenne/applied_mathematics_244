...
# ╔═╡ a6e12ecd-4278-42f6-ac46-888e2e320dc9
using PlutoUI, CairoMakie, DifferentialEquations, DataFrames, GLM, CSV, Polynomials

# ╔═╡ a2e18bf0-224d-11ed-0414-ed3babf21178
md"# Applied Mathematics 244 - Assignment 2"

# ╔═╡ 9cb8bd98-24ec-4305-8dd4-3faedec0c054
md"## Packages and data"

# ╔═╡ 84fa60f7-02e7-4397-9c95-e3a4112a0cb4
df = DataFrame(CSV.File("data.csv"))

# ╔═╡ 12d32ddb-3e5f-49ad-acbc-7cfc9294ddb8
years = df[:,1];

# ╔═╡ 19cbb9d8-0497-4848-a5f4-3016266beed7
pop = df[:,2];

# ╔═╡ 6c642c73-3064-4965-8dd7-92a45db4993a
md"## Analysis and figures"

# ╔═╡ fa714564-7175-46f9-b0a7-8a8f79ff2ef0
md"### Problem 1"

# ╔═╡ 50824b39-30e1-4c42-9ea6-f40ad5d1f876
real_t(year) = year/10 - 178;

# ╔═╡ d061da7e-b46d-4e6f-af17-7a4b2a7a8e96
t_real(t) = t*10 + 1780;

# ╔═╡ e7c030d4-3082-4a23-b656-f7519b610dd5
#Q(t) = 1/pop[t÷10] * (pop[t÷10 + 1] - pop[t÷10]) / 10
Q1(t) = 1/pop[t] * (pop[t+1] - pop[t]);

# ╔═╡ b99d5fd5-2453-4957-b217-4d10e43fb3f7
ols1 = lm(@formula(Population ~ Year), df)

# ╔═╡ 62cef962-3488-4170-b7b0-f632684f95c3
#save("fig1.pdf", f1)

# ╔═╡ 29589401-4c14-42fb-a3ea-519663d10c8a
#save("fig2.pdf", f2)

# ╔═╡ 1d2c3792-7206-465c-b73c-d2b371e72c78
p1 = Polynomials.fit(pop[1:16], Q1.(1:16), 1)

# ╔═╡ c68afab9-f246-4af4-b21e-ab2404ab3fb4
a1, b1 = p1.coeffs[1], -p1.coeffs[2]

# ╔═╡ 97a28e43-a0ee-4220-90e8-18efd8d69ba7
P1(t) = (a1*pop[1]) / (b1*pop[1] + ℯ^(-a1*(t-1)) * (a1 - b1*pop[1]));

# ╔═╡ b228a3e3-8385-4ab8-a836-efbb5f61c34c
K = a1 / b1

# ╔═╡ 99adcdab-e2a2-42e2-9c30-9ece5a269756
P1(real_t(2022))

# ╔═╡ 1318b142-8e2b-422d-81aa-a3c445204936
md"### Problem 2"

# ╔═╡ 79f3d3c0-747a-4777-8cfb-28bcac061ec0
p2 = Polynomials.fit(log.(pop[1:16]), Q1.(1:16), 1)

# ╔═╡ 1e3b0f10-de99-4fe9-a227-dc9313a36c5c
begin
	f2 = Figure()
	ax2 = Axis(f2[1, 1],
		xlabel = L"P(t)", 
	...
	l2_1 = lines!(ax2, x2, p1.(x2) , color = (:red, 0.8))
	l2_2 = lines!(ax2, x2, p2.(x2) , color = (:red, 0.8))
	f2
end

# ╔═╡ 82df6291-bd44-4039-8b1a-a7f066c748d4
a2, b2 = p2.coeffs[1], -p2.coeffs[2]

# ╔═╡ afbb8180-7d81-460b-b03e-641fed4cf79a
c2 = log(pop[1]) - a2 / b2

# ╔═╡ 7eb4c9ca-520b-49dc-bede-abc851fad8f4
P2(t) = ℯ^((a2/b2) + c2 * ℯ^(-b2 * t));

# ╔═╡ e07f2bff-c784-4324-8ce0-d3f2638e118a
begin
	f1 = Figure()
	ax1 = Axis(f1[1, 1],
		xlabel = L"t", 
	...
	l1_1 = lines!(ax1, x1, P1.(1:16), color = (:red, 0.8))
	l1_2 = lines!(ax1, x1, P2.(1:16), color = (:green, 0.8))
	legend_1 = axislegend(ax1, [l1_1, l1_2], ["Logistic", "Gompertz modified"], position = :rt)
	f1
end

# ╔═╡ c9253a68-573f-478c-928c-8f10d069c679
t_2(P) = log((log(P) - (a2/b2)) / c2) / -b2;

# ╔═╡ 95281f8c-c09e-4d52-9ffa-90c581d5a1a2
t_real(t_2(600))

# ╔═╡ dc6ae252-222b-4255-9b90-04c57f9d4331
md"#### Hidden definitions"

# ╔═╡ 763cec74-bcaa-45c2-b3ec-fd931aa1328d
TableOfContents()
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "21856aab6d3f67700f81eaccbfb4de3d5284cd4d"
