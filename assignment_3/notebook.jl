...
# ╔═╡ 24ba7ca1-d229-4386-94fd-b4c638210cd2
using PlutoUI, CairoMakie, DifferentialEquations

# ╔═╡ 7c1687a0-2de7-11ed-3b11-fb52ec9a1076
md"# Applied Mathematics 244 - Assignment 3"

# ╔═╡ b60c9733-2c1c-432e-9d5d-b531b30fd6d2
md"## Packages and data"

# ╔═╡ df5c866c-efc9-48e0-aef1-ae94cfd753cc
md"## Analysis and figures"

# ╔═╡ d04c758e-cb2e-404b-b49d-f40692867f6a
md"### Problem 1"

# ╔═╡ 066a0d81-575a-45d9-8da6-bae3d265d79b
md"#### (a)"

# ╔═╡ e1da6031-bda9-4856-9c14-9bb419ff5f89
...
# ╔═╡ 079ef90a-0f98-4593-a65e-738a54eb1c47
function f!(u)
	du1 = 4*u[1] + 2*u[2]
	du2 = 3*u[1] + 3*u[2]
	return [du1, du2]
...
# ╔═╡ 26c3e23b-37c0-4d59-b011-8b9d9639c8f3
begin
	u0_1 = [10.0;-5.0] # initial conditions
	tspan_1 = (0.0, 0.5) # timespan
	h = 0.01 # timestep
end

# ╔═╡ d98d94b2-e79e-401e-b219-910801afe256
e_sol = euler(f!, u0_1, tspan_1, h)

# ╔═╡ 26f6e2c1-610c-42c1-8e18-30705c48fbe5
e_sol1 = transpose(hcat(e_sol...))

# ╔═╡ 4981db55-fde4-4618-ba5d-665c06029129
md"#### (b)"

# ╔═╡ a37d1179-54d7-4fb7-b47b-3f66de3ed0d0
function f1!(du,u,p,t)
 du[1] = 4*u[1] + 2*u[2]
 du[2] = 3*u[1] + 3*u[2]
end

# ╔═╡ 7a97bfc4-d49e-4bcf-a408-9c43e738cc17
begin
	prob_1 = ODEProblem(f1!,u0_1,tspan_1)
	sol_1 = solve(prob_1, saveat = h, DP5())
end

# ╔═╡ 65d98102-e216-49b6-b34a-65288b8a17f6
md"#### (c)"

# ╔═╡ c1100c49-d5f9-4073-93fa-24e4f0ac3b7d
x₁(t) = 4*ℯ^(6*t) + 6*ℯ^t;

# ╔═╡ f7604257-05ba-4261-836c-0a0860e4785e
x₂(t) = 4*ℯ^(6*t) - 9*ℯ^t;

# ╔═╡ 2adddd34-6f36-4697-aa35-2c7de8bbc3ba
md"#### Final Plot"

# ╔═╡ ff732f89-c3b2-4e62-b1db-fa66f7a33879
t = sol_1.t;

# ╔═╡ 56cf3500-2e92-4174-9db9-c29777823588
begin
	f1 = Figure()
	ax1 = Axis(f1[1, 1],
		xlabel = L"x_1", 
	...
	#s1 = scatter!(ax1, e_sol1[:,1], e_sol1[:,2], marker = :xcross, markersize = 20)
	#s2 = scatter!(ax1, sol_1'[:,1], sol_1'[:,2])
	#l1 = lines!(ax1, x₁.(t), x₂.(t), color = (:red, 0.8))

	s1 = scatter!(ax1, e_sol1[:,1], e_sol1[:,2])
	s2 = scatter!(ax1, sol_1'[:,1], sol_1'[:,2], marker = :xcross, markersize = 20)
	l1 = lines!(ax1, x₁.(t), x₂.(t), color = (:red, 0.8))
	
	legend_1 = axislegend(ax1, [s1, s2, l1], [L"\mathrm{Euler's\; Method}", L"\mathrm{Dormand-Prince}", L"\mathrm{Analytic\; Solution}"], position = :rb)
	f1
end

# ╔═╡ 6cdaf50f-98da-4a98-ab8d-5e38d387ff56
begin
	f1_1 = Figure()
	ax1_1 = Axis(f1_1[1, 1],
		xlabel = L"t", 
		ylabel = L"x_1\; \mathrm{and}\; x_2"
		)
	#s1 = scatter!(ax1, e_sol1[:,1], e_sol1[:,2], marker = :xcross, markersize = 20)
	#s2 = scatter!(ax1, sol_1'[:,1], sol_1'[:,2])
	#l1 = lines!(ax1, x₁.(t), x₂.(t), color = (:red, 0.8))

	s1_1 = scatter!(ax1_1, t, e_sol1[:,1], color = (Makie.wong_colors()[1], 0.8))
	s1_2 = scatter!(ax1_1, t, e_sol1[:,2], color = (Makie.wong_colors()[1], 0.8))
	s2_1 = scatter!(ax1_1, t, sol_1'[:,1], color = (Makie.wong_colors()[2], 0.8), marker = :xcross, markersize = 20)
	s2_2 = scatter!(ax1_1, t, sol_1'[:,2], color = (Makie.wong_colors()[2], 0.8), marker = :xcross, markersize = 20)
	l1_1 = lines!(ax1_1, t, x₁.(t), color = (:red, 0.8))
	l1_2 = lines!(ax1_1, t, x₂.(t), color = (:red, 0.8))

	t1 = text!(0.2, 28, text = L"x_1", align = (:center, :center))
	t2 = text!(0.15, 5, text = L"x_2", align = (:center, :center))
	
	legend1_1 = axislegend(ax1_1, [s1_1, s2_1, l1_1], [L"\mathrm{Euler's\; Method}", L"\mathrm{Dormand-Prince}", L"\mathrm{Analytic\; Solution}"], position = :rb)
	f1_1
end

# ╔═╡ 7644cadb-790a-4c4b-b912-b2bfccff8dda
#save("f1.pdf", f1_1)

# ╔═╡ 0a1f8082-c7f6-41d4-9a0e-42a5e8f595f0
md"### Problem 2"

# ╔═╡ f9267c9f-3ab8-443e-8b43-e1be615452ed
function f2!(du,u,p,t)
 du[1] = 2*u[1] - 4*u[1]*u[2]
 du[2] = -3*u[2] + 4*u[1]*u[2]
end

# ╔═╡ 88a3af05-5d72-4bc4-9828-031d6a9f2b2f
begin
	u0_2 = [0.5, 1.5]
	tspan_2 = (0.0, 10.0)
	prob_2 = ODEProblem(f2!,u0_2,tspan_2)
	sol_2 = solve(prob_2, saveat = h, progress = true)
end

# ╔═╡ d648117e-5546-4033-aaaa-77e60b473206
begin
	f2 = Figure()
	ax2 = Axis(f2[1, 1],
	...
	l2_1 = lines!(ax2, sol_2.t, sol_2'[:,1])
	l2_2 = lines!(ax2, sol_2.t, sol_2'[:,2])
	#s2 = scatter!(ax1, x₁.(t), x₂.(t), marker = :xcross, markersize = 20)
	legend_2 = axislegend(ax2, [l2_1, l2_2], [L"\mathrm{Prey}", L"\mathrm{Predator}"], position = :rt)
	f2
end

# ╔═╡ a268c053-4a94-486b-a84e-7920f3b0cc88
#save("f2.pdf", f2)

# ╔═╡ 1695cd8b-10dd-4534-9dbf-c0dcce1d7f1a
extrema(sol_2'[:,1])

# ╔═╡ fbfdfadd-e2a9-4219-8f45-54e325b5bd60
extrema(sol_2'[:,2])

# ╔═╡ 3dffae93-4c5b-4ab2-82e5-462011099228
md"### Problem 3"

# ╔═╡ 481f531a-43aa-4ef0-bcbd-f79637adaa6c
function f3!(du,u,p,t)
 du[1] = -0.002*u[2]*u[1]
 du[2] = 0.002*u[2]*u[1] - 0.12*u[2]
 du[3] = 0.12*u[2]
end

# ╔═╡ dbc613af-bb9a-413c-b025-a396e305cdfc
begin
	u0_3 = [407.0, 3.0, 0.0]
	tspan_3 = (0.0, 28.0)
	prob_3 = ODEProblem(f3!,u0_3,tspan_3)
	sol_3 = solve(prob_3, saveat = h, progress = true)
end

# ╔═╡ a4b57b51-d1db-4b69-b4e2-62d67bc64bc9
begin
	f3 = Figure()
	ax3 = Axis(f3[1, 1],
		xlabel = L"\mathrm{Time\, (days)}", 
		ylabel = L"\mathrm{Population}"
		)
	l3_1 = lines!(ax3, sol_3.t, sol_3'[:,1])
	l3_2 = lines!(ax3, sol_3.t, sol_3'[:,2])
	l3_3 = lines!(ax3, sol_3.t, sol_3'[:,3])
	#s2 = scatter!(ax1, x₁.(t), x₂.(t), marker = :xcross, markersize = 20)
	legend_3 = axislegend(ax3, [l2_1, l2_2, l3_3], [L"\mathrm{Susceptible}", L"\mathrm{Infectious}", L"\mathrm{Recovered}"], position = :rt)
	f3
end

# ╔═╡ 875e2a72-263e-4b22-b9e8-e2dde0962b85
#save("f3.pdf", f3)

# ╔═╡ 53bdfc05-3524-4e44-b7e8-5194c5482841
maximum(sol_3'[:,2])

# ╔═╡ d72ef9b7-6688-4080-8868-918f29132ec2
sol_3'[:,2][end]

# ╔═╡ 2d0d9fc5-48da-4fe5-b6b7-0ee426455c0a
md"### Problem 4"

# ╔═╡ a1eeef5c-4cc2-4c2c-9cad-6f5fbf76ed9b
function f4!(du,u,p,t)
 du[1] = u[2]
 du[2] = 4.0*(1-u[1]^2)*u[2] - u[1]
end

# ╔═╡ 58442696-a120-4770-a27e-cb132143704e
begin
	u0_4 = [0.0, 1.0]
	tspan_4 = (0.0, 40.0)
	prob_4 = ODEProblem(f4!,u0_4,tspan_4)
	sol_4 = solve(prob_4, saveat = h, progress = true)
end

# ╔═╡ 3d8357fe-d68c-4ac1-b6ad-ad36b5dab9fa
begin
	f4_1 = Figure()
	ax4_1 = Axis(f4_1[1, 1],
		xlabel = L"t", 
	...
	s4_1 = lines!(ax4_1, sol_4.t, sol_4'[:,1])
	#s2_2 = lines!(ax2, sol_2.t, sol_2'[:,2])
	#s2 = scatter!(ax1, x₁.(t), x₂.(t), marker = :xcross, markersize = 20)
	#legend_2 = axislegend(ax2, [s2_1, s2_2], [L"Prey", L"Predator"], position = :rt)
	f4_1
end

# ╔═╡ ac2e17ae-0f97-4d0c-8ddd-4507f842f66c
#save("f4_1.pdf", f4_1)

# ╔═╡ b8728aed-99f9-45aa-8c1e-8c9bd7997836
begin
	f4_2 = Figure()
	ax4_2 = Axis(f4_2[1, 1],
		xlabel = L"x(t)", 
	...
	s4_2 = lines!(ax4_2, sol_4'[:,1], sol_4'[:,2])
	#s2_2 = lines!(ax2, sol_2.t, sol_2'[:,2])
	#s2 = scatter!(ax1, x₁.(t), x₂.(t), marker = :xcross, markersize = 20)
	#legend_2 = axislegend(ax2, [s2_1, s2_2], [L"Prey", L"Predator"], position = :rt)
	f4_2
...
# ╔═╡ 724c03d9-d2bc-4c77-9f74-c7103cc3c2cf
#save("f4_2.pdf", f4_2)

# ╔═╡ e36aacdd-a6ca-47e1-b8d0-8cee5d985257
md"#### Hidden definitions"

# ╔═╡ c38a5916-36ef-41c2-8ca9-ef39f0ef3fe3
TableOfContents()
...
julia_version = "1.8.0"
manifest_format = "2.0"
project_hash = "339512c73ec7ed59a955f5f2ef55b4fdba1d7d8b"
